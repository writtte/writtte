...
CREATE OR REPLACE FUNCTION schema_main.v1_subscription_update (p_data JSONB)
...
BEGIN
  ...
  END IF;
  RETURN json_build_object(k_status, TRUE, k_code, 'SUBSCRIPTION_UPDATED', k_message, NULL, k_additional, NULL, k_data, json_build_object('id_main', v_updated_id, 'account_code', v_p_account_code, 'status', v_p_status, 'updated_time', v_updated_time)::JSONB)::JSONB;
EXCEPTION
  WHEN OTHERS THEN
