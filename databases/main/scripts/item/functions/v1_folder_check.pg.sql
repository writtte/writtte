\echo create function schema_item.v1_folder_check()
CREATE OR REPLACE FUNCTION schema_item.v1_folder_check (p_data JSONB, p_is_for_exists BOOLEAN)
...
BEGIN
  ...
  SELECT
    EXISTS (
      ...
      WHERE (v_p_folder_code IS NOT NULL
        AND folder_code = v_p_folder_code)
      OR (v_p_account_code IS NOT NULL
        AND account_code = v_p_account_code)) INTO v_folder_exists;
  IF p_is_for_exists AND v_folder_exists THEN
    RETURN json_build_object(k_status, TRUE, k_code, 'FOLDER_EXISTS', k_message, NULL, k_additional, NULL, k_data, NULL)::JSONB;
  ELSIF p_is_for_exists
      AND NOT v_folder_exists THEN
      RETURN json_build_object(k_status, TRUE, k_code, 'FOLDER_NOT_EXISTS', k_message, NULL, k_additional, NULL, k_data, NULL)::JSONB;
  ELSIF NOT p_is_for_exists
      AND v_folder_exists THEN
      RETURN json_build_object(k_status, TRUE, k_code, 'FOLDER_EXISTS', k_message, NULL, k_additional, NULL, k_data, NULL)::JSONB;
  ELSIF NOT p_is_for_exists
      AND NOT v_folder_exists THEN
      RETURN json_build_object(k_status, TRUE, k_code, 'FOLDER_NOT_EXISTS', k_message, NULL, k_additional, NULL, k_data, NULL)::JSONB;
