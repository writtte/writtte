\echo create function schema_item.v1_tree_add_document()
CREATE OR REPLACE FUNCTION schema_item.v1_tree_add_document (p_data JSONB)
...
BEGIN
  ...
  IF v_tree_exists THEN
    RETURN json_build_object(k_status, TRUE, k_code, 'TREE_ALREADY_EXISTS', k_message, NULL, k_additional, NULL, k_data, NULL)::JSONB;
  END IF;
  INSERT INTO schema_item.tb_tree (folder_code, document_code)
    VALUES (v_p_folder_code, v_p_document_code);
  RETURN json_build_object(k_status, TRUE, k_code, 'TREE_DOCUMENT_ADDED', k_message, NULL, k_additional, NULL, k_data, json_build_object('folder_code', v_p_folder_code, 'document_code', v_p_document_code)::JSONB)::JSONB;
EXCEPTION
  WHEN OTHERS THEN
