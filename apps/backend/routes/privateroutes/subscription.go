...
import (
	...
	"backend/apis/v1/subscription/v1subscriptionretrieve"
	"backend/cmd/glob"
	"backend/configs"
	"backend/constants"
	"backend/middleware"
...
func Subscription(mux *http.ServeMux, prefix *string) {
	basePrefix := *prefix + "/subscription"

	subscriptionGet(mux, &basePrefix)
}

func subscriptionGet(mux *http.ServeMux, prefix *string) {
	var (
		subscriptionPath = ""
	)
	...
	flows := []middleware.Flow{
		{
			Handler: v1subscriptionretrieve.Setup(),
